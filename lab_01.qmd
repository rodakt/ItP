...
Variables in Python are **not** typed. The same variable can hold values of different types at different times.
...
The `and` and `or` operators, on the other hand, do **not** always return a Boolean value. Instead, they return one of their operands—the first one that determines the outcome of the operation.

### Exercise 1.9

Explain the rationale behind the expression `not not x`. What effect does it have on `x`?
